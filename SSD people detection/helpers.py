...
def draw_box_label(id,img, bbox_cv2, tot_LTR, tot_RTL, box_color=(0, 255, 255), show_label=True):
    '''
    ...
    if show_label:
        ...
        ##### Drawing dot of the subject
        x_pos = int((left+right)/2)
        y_pos = int((top+bottom)/2)
        cv2.circle(img,(x_pos, y_pos), 10, (0,0,255), -1)

        ##### Drawing center line
        height, width, channels = img.shape
        center = int(width/2)
        start_pt = (center, int(height))
        end_pt = ((int(width/2), 0))
        cv2.line(img, start_pt, end_pt, (0, 0, 255), 3)

        # Output the labels that show the x and y coordinates of the bounding box center.
        text_id= 'id='+str(id)
        cv2.putText(img,text_id,(left,top-25), font, font_size, font_color, 1, cv2.LINE_AA)
        text_pos= f"(x,y): ({x_pos}, {y_pos})"
        cv2.putText(img,text_pos,(left,top-5), font, font_size, font_color, 1, cv2.LINE_AA)

        ##### Output the amount of people displacement

        ### LTR
        text_LTR = f"LTR: {tot_LTR}"
        cv2.putText(img,text_LTR,(10,height-3), font, 1, font_color, 1, cv2.LINE_AA)

        ### RTL
        text_RTL = f"RTL: {tot_RTL}"
        cv2.putText(img,text_RTL,(width-200,height-3), font, 1, font_color, 1, cv2.LINE_AA)
    
    return img, x_pos, center 